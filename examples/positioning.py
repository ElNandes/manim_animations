...
class Positioning(Scene):
    def construct(self):
        ...
        blue_dot.next_to(red_dot, RIGHT + UP)
...
class Grouping(Scene):
    def construct(self):
        ...
        #* is added before [] vecause VGroup does not accept list and 
        #therefore an unpacked list has to be passed.
        circle =  VGroup(*[Circle(radius=0.2) for _ in range(10)])
        circle.arrange(UP, buff=0.3)
        self.add(circle)

        stars = VGroup(*[Star(color=YELLOW, fill_opacity=1).scale(0.2) for _ in range(20)])
